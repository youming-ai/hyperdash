...
export const userRouter = new Hono<AppEnv>()
  ...
  .get('/wallets', requireSession, async (c) => {
    ...
    const db = c.get('db');
    const userId = await resolveBusinessUserId(db, session.walletAddress);
    return c.json(mockWallets(userId).map((wallet: unknown) => AgentWallet.parse(wallet)));
  })
  ...
  .get('/alerts', requireSession, zValidator('query', alertsQuery), async (c) => {
    ...
    return c.json(filtered.slice(0, limit).map((alert: unknown) => PriceAlert.parse(alert)));
  })
  ...
  .get('/notifications', requireSession, zValidator('query', notificationsQuery), async (c) => {
    ...
    if (type !== 'all')
      filtered = filtered.filter(
        (notif: unknown) =>
          typeof notif === 'object' && notif !== null && 'type' in notif && notif.type === type,
      ) as typeof filtered;
    if (read !== undefined)
      filtered = filtered.filter(
        (notif: unknown) =>
          typeof notif === 'object' && notif !== null && 'read' in notif && notif.read === read,
      ) as typeof filtered;
    const paginated = filtered.slice(offset, offset + limit);
    return c.json({
      notifications: paginated.map((notif: unknown) => Notification.parse(notif)),
      unread: filtered.filter(
        (notif: unknown) =>
          typeof notif === 'object' && notif !== null && 'read' in notif && !notif.read,
      ).length,
    });
  })
  .post(
  ...
  .get('/trading-history', requireSession, zValidator('query', tradingHistoryQuery), async (c) => {
    ...
    return c.json(mockHistory.map((trade: unknown) => UserTrade.parse(trade)));
  })
